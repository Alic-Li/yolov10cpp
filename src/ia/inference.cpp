 ...
 */
cv::Mat letterbox(const cv::Mat &src, const cv::Size &target_size, const cv::Scalar &color = cv::Scalar(114, 114, 114))
{
      ...
      input_shape{1, 3, 1024, 1024}
{
    ...
    auto model_input_shape = session.GetInputTypeInfo(0).GetTensorTypeAndShapeInfo().GetShape();
    if (model_input_shape.size() == 4 && model_input_shape[2] > 0 && model_input_shape[3] > 0)
    {
